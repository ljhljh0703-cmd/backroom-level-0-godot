...
| B | 탈출 후 다시 백룸 | 필요한 단서를 다 얻지 못한 상태에서 가짜 출구 방으로 몰려 들어감. |
...
| 8 | `false_exit_room` | B 엔딩 후보 | 단서 부족 시 크리처에게 쫓기며 들어가는 가짜 출구 |
...
7. B 가짜 출구는 단서 부족 상태에서 크리처에게 쫓기며 이판사판으로 들어가는 플레이 경험으로 디벨롭한다.
...
- B 가짜 출구는 "단서를 충분히 얻지 못해서 크리처에게 쫓기며 급하게 들어가는 방"으로 디벨롭한다.
...
GDD v2는 승인되었다. 다음 단계는 개발 요구사항 문서화다.

개발 요구사항 단계에서 확정할 항목:

1. 8개 방의 정확한 route graph.
2. 각 방의 hotspot 목록과 판정 크기.
3. 좌측 전등 버튼 작동 후 열리는 정보/이동 변화.
4. A 진짜 출구 진입 조건.
5. B 가짜 출구 진입 조건과 추격감 연출 범위.
