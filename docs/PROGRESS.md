...
| 2026-06-14 | `right_dead_end` 오른쪽 문을 1회 경고 후 2회 클릭 시 B 전환으로 바꿨고, 경고 후 복귀 시 쪽지 힌트를 표시한다. | B 전환을 즉시 처벌이 아니라 경고 무시 결과로 만들고, A 루트 힌트를 우측 위험 루트의 보상으로 주기 위해. |
| 2026-06-14 | 쪽지는 클릭 시 별도 화면으로 열고, 화면 바깥 클릭 시 읽은 것으로 처리하며 0.5초 긴장 이미지를 띄운다. | 힌트 읽기 완료 시점을 플레이어의 닫기 행동에 맞추기 위해. |
| 2026-06-14 | A 엔딩 조건에서 `blood_trace_clicked`를 제외하고, 스위치 작동 후 밝아진 STOP 뒤 공간이 바로 `true_exit_room`으로 이어지게 했다. | A 루트가 단서 누락으로 막힌 것처럼 느껴지는 문제를 없애고 쪽지 힌트와 실제 동선을 일치시키기 위해. |
| 2026-06-24 | 포트폴리오 카피를 `humanize-korean` 1차 후 `juhyeong-voice` 톤으로 다시 윤문했다. | 번역투를 걷어내고, 실패와 수정 과정이 보이는 짧은 호흡의 포폴 문장으로 맞추기 위해. |
